...
| 기능 | 설명 |
|------|------|
| **스캔 수신 (R01)** | 모바일에서 업로드한 LiDAR 파일(.zip)을 Cloudflare R2에 저장하고 AI 서버에 3D 재구성을 요청합니다. |
| **하자 탐지 (D01)** | 단일 방 스캔을 AI 서버에 분석 요청해 하자 종류·심각도·3D 좌표를 수신하고, 단가 테이블 기반으로 수리 예상 비용을 산정합니다. |
| **하자 비교 (D02)** | 입주 전·퇴거 후 두 스캔을 비교해 새로 생긴 하자만 추출합니다. |
| **수리 업체 연결** | 방 주소를 기반으로 Kakao Local API에서 주변 수리 업체를 검색하고 견적 요청 이력을 저장합니다. |
| **수리 이력 관리** | 수리 완료 후 업체명·비용·메모를 기록하고 연관 견적 상태를 COMPLETED로 갱신합니다. |
