...
main_room_id int [ref: > Room.room_id, null] // 대표 방 ID
...
room_id int [ref: > Room.room_id] // 비교 대상 방 ID
in_scan_id int [ref: > Scan.scan_id] // 입주 스캔 ID
out_scan_id int [ref: > Scan.scan_id] // 퇴거 스캔 ID
...
user_id int [ref: > User.user_id] // 사용자 FK
room_id int [ref: > Room.room_id] // 방 FK
analysis_id int [ref: > Analysis.analysis_id] // 분석 결과 FK
...
room_id int [ref: > Room.room_id] // 방 FK
estimate_id int [ref: > Estimate.estimate_id, null] // 연결된 견적 FK
provider_name varchar // 업체명
repair_cost int // 실제 수리 비용
status varchar // IN_PROGRESS / COMPLETED
repaired_at datetime [null] // 수리 완료 시각
